...
4. 予算を減らす方法は一つではない
5. どの削減方法を選ぶかによって、住民サービスへの影響が異なる
...
- スライダーが分野ごとに独立しており、年間総予算を固定した分野間の再配分になっていない
- 現在の財源不足・余裕表示は暫定仕様であり、将来は減額分を「配分可能額」として別分野へ移す方式に変更する
- 分野別の詳細ページがない
...
ルーティング方式は実装環境に合わせて変更可能。利用者の設定額を引き継げることを必須とする。
...
総額を維持するには、他分野から同額を減額するか、別の財源を確保する必要があります。
```
...
| 予算分類と所管局が一致しない | 「主な所管」として表示し、集計範囲の違いを明示 |
| 全9分野の調査が終わらない | 3分野を深く完成し、残りは基礎情報に限定 |

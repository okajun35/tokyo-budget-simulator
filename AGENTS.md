...
- The last verification passed 235 tests, production build, ESLint, Cloudflare deployment-contract validation, and desktop/mobile layout and heading audits over 14 stateful and primary paths. The simulator audit covers whole-card pointer selection, keyboard selection, keyboard slider operation, the result CTA that appears in the fixed budget bar after a change, and the allocation-result route after a complete redistribution. The result summary lists only changed fields, distinguishes reallocated from still-unallocated money, explains what each changed field funds using existing category data, and preserves all nine allocations in category-detail links. A category detail opened from that result returns to the same result with the complete plan; this origin is retained through the case and material pages, while direct or simulator-origin detail links still return to the simulator. On returning to the result, the originating changed-field card is scrolled fully into view and receives a visible programmatic focus without becoming an extra Tab stop; the browser audit checks this flow at desktop and 390px widths. The participation audit also covers the optional AI disclosure, default-off consent state, and the selected official contact repeated after the drafting and AI workflow. At 900px and below, the shared header keeps the brand visible and replaces the five compressed links with an accessible menu button; the layout audit checks one-line 48px menu links, no horizontal overflow, and Escape focus return at 390px.
